...
export const createWallet = async (formData) => {
    const { wallet_name, wallet_address, network_type, customer_name, id } = Object.fromEntries(formData);
    ...
    try {
        ...
        if (authenticatedUser?.withdrawal === null) {
            const walletData = {
                ...
                customer_name: authenticatedUser?.customer_name,
                status: "pending"
            };
...
export const resetPassword = async (formData) => {
    ...
    try {
        ...
        if (new_password !== confirm_password) return {
            message: `Inconsistent entry of new password!`,
            status: 404,
            type: "danger"
        };
